...
<%
	...
	if(request.getParameter("startRecord") != null){//First entry		
        ...
        ConnRecordset1.prepareStatement("SELECT count(*) FROM DataType");  
        ResultSet CountRecordset1 = preCountStatement.executeQuery();
        CountRecordset1.next();
        totalRecords = CountRecordset1.getInt(1);                
    }    
	
PreparedStatement StatementRecordset1 = ConnRecordset1.prepareStatement("SELECT * FROM DataType, UsersTable where DataType.UserID=UsersTable.UserID order by DataTypeID desc limit "+start+","+showRecords+"; ");


if(dbServerProduct.equals("SQLServer2012")){
  StatementRecordset1 = ConnRecordset1.prepareStatement("SELECT * FROM DataType, UsersTable where DataType.UserID=UsersTable.UserID order by DataTypeID desc OFFSET  "+start+" ROWS FETCH NEXT "+showRecords+" ROWS ONLY;");
}
else if(dbServerProduct.equals("SQLServer2008")){
  StatementRecordset1 = ConnRecordset1.prepareStatement("SELECT * FROM (SELECT ROW_NUMBER() OVER ( ORDER BY DataTypeID ) AS RowNum, * FROM DataType, UsersTable where DataType.UserID=UsersTable.UserID) AS RowConstrainedResult where RowNum >= "+start+" and RowNum < "+(start+showRecords)+" ORDER BY RowNum; ");
}
...
<H2>DataType</H2>
<a href="DataTypeInput.jsp"><img src="../images/add_btn.png" alt="" width="95" height="34" align="left" /></a>
...
%>
    	   ...
    	   <tr>
				...
				<td><a href='DataTypeUpdate.jsp?DataTypeID=<%=Recordset1.getObject("DataTypeID") %>'>Update</a> <a href='#' onclick="deleteOnClick('DataTypeDelete.jsp?DataTypeID=<%=Recordset1.getObject("DataTypeID") %>')">Delete</a></td>
    	   </tr>	
...
%>
    ...
    <div align="center">
      <label></label>
      <%=pageIndex1.traverseBar("DataTypeMain.jsp", "startRecord", totalRecords, showRecords, start) %>
    </div>
    </label>
    <a href="DataTypeInput.jsp"><img src="../images/add_btn.png" width="95" height="34" /></a>    
