...
<%
  ...
  int totalRecords = 0;
    ...
    ConnRecordset1.prepareStatement("SELECT count(*) FROM Region, Users where SexTypeName like ?");
    ...
    StatementRecordset1 = ConnRecordset1.prepareStatement("SELECT * FROM Region, Users where SexTypeName like ? order by SexTypeID limit "+start+","+showRecords+"; ");

    if(dbServerProduct.equals("SQLServer2012")){
      StatementRecordset1 = ConnRecordset1.prepareStatement("SELECT * FROM Region, Users where SexTypeName like ? order by SexTypeID desc OFFSET  "+start+" ROWS FETCH NEXT "+showRecords+" ROWS ONLY;");
    }
    else if(dbServerProduct.equals("SQLServer2008")){
      StatementRecordset1 = ConnRecordset1.prepareStatement("SELECT * FROM (SELECT ROW_NUMBER() OVER ( ORDER BY SexTypeID ) AS RowNum, * FROM Region, Users where SexTypeName like ?) AS RowConstrainedResult where RowNum >= "+start+" and RowNum < "+(start+showRecords)+" ORDER BY RowNum; ");
    }
