...
<%
	...
	if(request.getParameter("startRecord") != null){//First entry		
        ...
        ConnRecordset1.prepareStatement("SELECT count(*) FROM Statistic");  
        ResultSet CountRecordset1 = preCountStatement.executeQuery();
        CountRecordset1.next();
        totalRecords = CountRecordset1.getInt(1);                
    }    
	
PreparedStatement StatementRecordset1 = ConnRecordset1.prepareStatement("SELECT * FROM Statistic, UsersTable where Statistic.UserID=UsersTable.UserID order by StatisticID desc limit "+start+","+showRecords+"; ");


if(dbServerProduct.equals("SQLServer2012")){
  StatementRecordset1 = ConnRecordset1.prepareStatement("SELECT * FROM Statistic, UsersTable where Statistic.UserID=UsersTable.UserID order by StatisticID desc OFFSET  "+start+" ROWS FETCH NEXT "+showRecords+" ROWS ONLY;");
}
else if(dbServerProduct.equals("SQLServer2008")){
  StatementRecordset1 = ConnRecordset1.prepareStatement("SELECT * FROM (SELECT ROW_NUMBER() OVER ( ORDER BY StatisticID ) AS RowNum, * FROM Statistic, UsersTable where Statistic.UserID=UsersTable.UserID) AS RowConstrainedResult where RowNum >= "+start+" and RowNum < "+(start+showRecords)+" ORDER BY RowNum; ");
}
...
<H2>Statistic</H2>
<a href="StatisticInput.jsp"><img src="../images/add_btn.png" alt="" width="95" height="34" align="left" /></a>
...
%>
    	   ...
    	   <tr>
				...
				<td><a href='StatisticUpdate.jsp?StatisticID=<%=Recordset1.getObject("StatisticID") %>'>Update</a> <a href='#' onclick="deleteOnClick('StatisticDelete.jsp?StatisticID=<%=Recordset1.getObject("StatisticID") %>')">Delete</a></td>
    	   </tr>	
...
%>
    ...
    <div align="center">
      <label></label>
      <%=pageIndex1.traverseBar("StatisticMain.jsp", "startRecord", totalRecords, showRecords, start) %>
    </div>
    </label>
    <a href="StatisticInput.jsp"><img src="../images/add_btn.png" width="95" height="34" /></a>    
