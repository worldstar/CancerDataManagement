...
<%
	...
	if(request.getParameter("startRecord") != null){//First entry		
        ...
        ConnRecordset1.prepareStatement("SELECT count(*) FROM sysdiagrams");  
        ResultSet CountRecordset1 = preCountStatement.executeQuery();
        CountRecordset1.next();
        totalRecords = CountRecordset1.getInt(1);                
    }    
	
PreparedStatement StatementRecordset1 = ConnRecordset1.prepareStatement("SELECT * FROM sysdiagrams order by name desc limit "+start+","+showRecords+"; ");


if(dbServerProduct.equals("SQLServer2012")){
  StatementRecordset1 = ConnRecordset1.prepareStatement("SELECT * FROM sysdiagrams order by name desc OFFSET  "+start+" ROWS FETCH NEXT "+showRecords+" ROWS ONLY;");
}
else if(dbServerProduct.equals("SQLServer2008")){
  StatementRecordset1 = ConnRecordset1.prepareStatement("SELECT * FROM (SELECT ROW_NUMBER() OVER ( ORDER BY name ) AS RowNum, * FROM sysdiagrams) AS RowConstrainedResult where RowNum >= "+start+" and RowNum < "+(start+showRecords)+" ORDER BY RowNum; ");
}
...
<H2>sysdiagrams</H2>
<a href="sysdiagramsInput.jsp"><img src="../images/add_btn.png" alt="" width="95" height="34" align="left" /></a>
...
%>
    	   ...
    	   <tr>
				...
				<td><a href='sysdiagramsUpdate.jsp?name=<%=Recordset1.getObject("name") %>'>Update</a> <a href='#' onclick="deleteOnClick('sysdiagramsDelete.jsp?name=<%=Recordset1.getObject("name") %>')">Delete</a></td>
    	   </tr>	
...
%>
    ...
    <div align="center">
      <label></label>
      <%=pageIndex1.traverseBar("sysdiagramsMain.jsp", "startRecord", totalRecords, showRecords, start) %>
    </div>
    </label>
    <a href="sysdiagramsInput.jsp"><img src="../images/add_btn.png" width="95" height="34" /></a>    
