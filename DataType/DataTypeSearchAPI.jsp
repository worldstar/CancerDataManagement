...
<%
  ...
  int totalRecords = 0;
    ...
    ConnRecordset1.prepareStatement("SELECT count(*) FROM DataType, Users where DataTypeName like ?");
    ...
    StatementRecordset1 = ConnRecordset1.prepareStatement("SELECT * FROM DataType, Users where DataTypeName like ? order by DataTypeID limit "+start+","+showRecords+"; ");

    if(dbServerProduct.equals("SQLServer2012")){
      StatementRecordset1 = ConnRecordset1.prepareStatement("SELECT * FROM DataType, Users where DataTypeName like ? order by DataTypeID desc OFFSET  "+start+" ROWS FETCH NEXT "+showRecords+" ROWS ONLY;");
    }
    else if(dbServerProduct.equals("SQLServer2008")){
      StatementRecordset1 = ConnRecordset1.prepareStatement("SELECT * FROM (SELECT ROW_NUMBER() OVER ( ORDER BY DataTypeID ) AS RowNum, * FROM DataType, Users where DataTypeName like ?) AS RowConstrainedResult where RowNum >= "+start+" and RowNum < "+(start+showRecords)+" ORDER BY RowNum; ");
    }
